 ...
 * @author S331473306
 */
public class PoliceGhost extends Player implements Runnable{
    ...
    public PoliceGhost(int lives, int xAxis, int yAxis, String image, boolean isAlive, Level1 lvl1, Player pacman) {
    ...
    public void run() {
       while(true){
           ...
           if(getxAxis() <= 260){
               direction = 1;
               setImage("images//policeGhostRight.png");
           
           }else if(getxAxis() >= 520){
               direction = -1;
               setImage("images//policeGhostLeft.png");
               
           }
            kill();
            setxAxis(getxAxis() + 20 * direction);
            lvl1.repaint();
       }
    }
